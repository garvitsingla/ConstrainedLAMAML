...
p.add_argument("--adapt-lr", type=float, default=0.5, help="Learning rate (step size) for online adaptation")
p.add_argument("--adapt-steps", type=int, default=1, help="Number of gradient updates during online adaptation (0 to disable)")
p.add_argument("--adapt-episodes", type=int, default=10, help="Number of support episodes to collect for adaptation")
p.add_argument("--max-kl", type=float, default=0.01, help="Maximum KL divergence constraint for adaptation line search")
args = p.parse_args()
...
# ─────────────────────────────────────────────────────────────────────────────
# Online Adaptation helper
# ─────────────────────────────────────────────────────────────────────────────
def adapt_policy_online(env, policy, initial_params, mission, seed=None, lr=0.5, num_steps=1, num_episodes=10, gamma=0.95, max_kl=0.01):
    """
    Perform few-shot online adaptation during inference on a given environment instance.
    Uses LinearFeatureBaseline to compute GAE advantages and cost advantages,
    performs SGD updates via policy.update_params, and uses line search to enforce a KL constraint.
    """
    if num_steps <= 0 or num_episodes <= 0:
    ...
    # Get input size for baseline
    with silence():
        dummy_obs, _ = env.reset(seed=seed)
    obs_dim = preprocess_obs(dummy_obs).shape[0]

    # Initialize baselines
    baseline = LinearFeatureBaseline(obs_dim).to(device)
    cost_baseline = LinearFeatureBaseline(obs_dim).to(device)

    # Constraint penalty values
    lambda_weights = {2: 0.8, 3: 0.3, 4: 0.5}  # 2: lava, 3: grass, 4: water
    HAZARD_MAP = {'lava': 2, 'grass': 3, 'water': 4}

    # Determine constraint multiplier based on mission text
    active_lambda = 0.5
    for hazard, idx in HAZARD_MAP.items():
        if f"avoid {hazard}" in mission:
            active_lambda = lambda_weights.get(idx, 0.5)
            break

    # Initialize adapted params
    params = OrderedDict((k, v.clone().detach().requires_grad_(True)) for k, v in initial_params.items())

    for step in range(num_steps):
        # Ensure parameters require grad at the start of the step
        params = OrderedDict((k, v.clone().detach().requires_grad_(True)) for k, v in params.items())

        # 1. Collect support trajectories under current params
        episodes = BatchEpisodes(batch_size=num_episodes, gamma=gamma, device=device)
        ...
        for ep_idx in range(num_episodes):
            ...
            obs_list, action_list, reward_list, cost_list = [], [], [], []
            while not done:
                obs_t = preprocess_obs(obs)
                obs_tensor = torch.from_numpy(obs_t[None]).float().to(device)
            ...
            # Convert to numpy arrays to match SamplerWorker structure
            obs_arr = np.array(obs_list, dtype=np.float32)
            act_arr = np.array(action_list, dtype=np.float32)
            rew_arr = np.array(reward_list, dtype=np.float32)
            cst_arr = np.array(cost_list, dtype=np.float32)
            batch_ids = [ep_idx] * len(obs_list)

            # Append using BatchEpisodes format
            episodes.append(obs_arr, act_arr, rew_arr, cst_arr, batch_ids)

        # 2. Fit baselines and compute advantages
        baseline.fit(episodes)
        cost_baseline.fit_costs(episodes)

        # Compute advantages
        episodes.compute_advantages(baseline, gae_lambda=1.0, normalize=True)
        episodes.compute_cost_advantages(cost_baseline, gae_lambda=1.0, normalize=True)

        # 3. Compute REINFORCE loss with combined advantages
        combined_adv = episodes.advantages - active_lambda * episodes.cost_advantages

        # Flatten inputs for batch forward pass
        flat_obs = episodes.observations.view((-1, episodes.observation_shape[0]))
        flat_actions = episodes.actions.view((-1,))
        
        # Compute policy probability under current params
        pi = policy(flat_obs, params=params)
        log_probs = pi.log_prob(flat_actions).view(len(episodes), episodes.batch_size)
        
        # Loss calculation (negative because we want to maximize returns)
        loss = -weighted_mean(log_probs * combined_adv, lengths=episodes.lengths).mean()

        # 4. Compute gradients with respect to params_before
        params_before = OrderedDict((k, v.clone().detach().requires_grad_(True)) for k, v in params.items())
        grads = torch.autograd.grad(loss, params_before.values(), allow_unused=True)
        
        # Enforce KL constraint via line search
        step_size = lr
        
        # Backtracking loop (max 10 iterations)
        for ls_step in range(10):
            # Compute updated_params manually to handle unused gradients gracefully
            updated_params = OrderedDict()
            for (name, param), grad in zip(params_before.items(), grads):
                if grad is not None:
                    updated_params[name] = param - step_size * grad
                else:
                    updated_params[name] = param.clone()
            
            # Compute KL divergence on the collected trajectories
            with torch.no_grad():
                pi_before = policy(flat_obs, params=params_before)
                pi_after = policy(flat_obs, params=updated_params)
                kl_flat = kl_divergence(pi_before, pi_after)
                kl_2d = kl_flat.view(len(episodes), episodes.batch_size)
                kl = weighted_mean(kl_2d, lengths=episodes.lengths).mean().item()
            
            # If KL is within limits, accept the update and exit line search
            if kl <= max_kl:
                params = updated_params
                break
            else:
                # Backtrack: decrease step size
                step_size *= 0.5
        else:
            # Keep parameters unchanged if line search failed to find a valid update
            params = params_before

    return OrderedDict((k, v.detach()) for k, v in params.items())
...
for dt, dc in DELTA_PAIRS:
    ...
    for c_room, c_dists in configs:
        ...
        for mission in test_tasks:
            ...
            for ep in range(n_episodes):
                ...
                adapted_params_clamaml = adapt_policy_online(
                    env, policy_clamaml, theta_prime, mission, seed=ep_seeds[ep],
                    lr=args.adapt_lr, num_steps=args.adapt_steps, num_episodes=args.adapt_episodes,
                    max_kl=args.max_kl
                )
                s, ok, v = evaluate_policy(env, policy_clamaml, params=adapted_params_clamaml, seed=ep_seeds[ep])
    ...
    xlsx_path = "few_shot_crl_summary_results.xlsx"
